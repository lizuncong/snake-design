...
class UIUXSearchEngine {
  ...
  generateDesignSystem(query, projectName = null) {
    ...
    console.log("[UIUXSearch:DIAG-Step1] category=" + category + ", candidates=[" + productResults.map(function(r){return r["Product Type"]}).join("|") + "]");
    // Step 2: 获取推理规则
...
// 脚本执行后根据 _params 自动分发到对应方法，返回 JSON 字符串
 return (function dispatchFromParams() {
  ...
  // 返回 JSON 字符串作为 eval 结果

  const res = typeof result === 'string' ? result : JSON.stringify(result, null, 2);
  console.log(\`[UIUXSearch:dispatch] mode=\${mode}, output_size=\${res.length} chars\`);
  // === DIAG ===
  console.log("[DIAG] result_type=" + typeof result + " len=" + (typeof result === "string" ? String(result.length) : "-") + " query=" + (_params.query || "").substring(0,40));
  return res
})();
`;
