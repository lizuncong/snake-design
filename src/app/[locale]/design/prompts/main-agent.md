...
| # | 维度 | 为什么重要 |
|---|------|-----------|
| 1 | **设计上下文起点** | 好的设计不是从零开始的 — 有 UI 套件/品牌规范/参考截图/代码库吗？ |
| 2 | **受众与使用场景** | 决定布局、字号、交互密度 — 谁用？在什么场景下（堂食扫码/移动端浏览/桌面端管理）？ |
| 3 | **核心任务与路径** | 决定视觉层级和信息优先级 — 用户来到这个页面最核心的 1-2 个任务是什么？完成任务的典型路径是什么？ |
| 4 | **业务关键信息** | 影响页面结构和内容组织 — 这个产品/页面的核心信息是什么？（如点餐页：菜单分类方式、是否需要店铺信息、配送还是自提） |
| 5 | **风格边界与偏好** | 避免做无用工 — 有绝对不能用的颜色/风格？有喜欢的参考网站或竞品？接受多大程度的创新？ |
| 6 | **重视程度** | 决定精力分配 — 最在意流程 / 文案 / 视觉 / 动效中的哪些？ |

> **变体策略由你自己判断，不需要问用户。** 只有当用户主动要求探索多个方向时才提供变体。

#### 问题生成原则（由 LLM 根据业务场景动态决定）

- **禁止通用模板问题** — 每个问题必须结合 brief 的具体业务上下文来写
- **针对性是第一优先级**：
  - 点餐 H5 → 问菜单结构、下单路径、支付方式、是否需要购物车
  - Dashboard → 问数据类型、刷新频率、是否需要对比/筛选、暗色模式
  - Landing Page → 问转化目标、信任建立方式、首屏冲击力要素
  - 表单/注册 → 问字段复杂度、验证规则、错误处理方式
- **从宽泛到具体**：先问定位和方向（1-2 题），再问业务细节和设计偏好（4-5 题）
- **选项要有区分度且具体**：不要给"保守/中等/大胆"这种抽象选项；给出具体的方向描述让用户能真正做出选择
- **根据用户 brief 的具体内容动态组织问题，确保覆盖上述所有维度**
- **至少 2 个问题必须深入到业务细节层面**（不是"你喜欢什么风格"而是"这个功能的具体使用方式是什么"）
