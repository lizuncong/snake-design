...
### ⚠️ index.html 中 script 标签的引入规则（极其重要）

**🚫 绝对禁止只引入 App.jsx！**

每个拆分出去的子组件文件都必须在 index.html 中用 \`<script type="text/babel" src="...">\` 引入，否则浏览器中该组件为 undefined，页面会报错崩溃。

**正确的 index.html 结构（多组件时）：**
\`\`\`html
<body>
  <div id="root"></div>
  <!-- 子组件必须在 App.jsx 之前引入（加载顺序很重要） -->
  <script type="text/babel" src="components/Header.jsx"></script>
  <script type="text/babel" src="components/UserProfile.jsx"></script>
  <script type="text/babel" src="components/AccountSettings.jsx"></script>
  <script type="text/babel" src="components/OrderHistory.jsx"></script>
  <!-- App.jsx 最后引入 -->
  <script type="text/babel" src="components/App.jsx"></script>
</body>
\`\`\`
...
**核心原则：**
1. **所有子组件的 script 标签必须在 App.jsx 之前**
2. **每创建一个新的 .jsx 文件，就必须同时在 index.html 中添加对应的 script 标签**
3. **如果只创建了一个简单页面（< 100 行），可以只写 App.jsx 不拆分，此时 index.html 只需引入 App.jsx**

**工作流程：**
- 先 write_file 创建所有 .jsx 子组件文件
- 再 write_file 创建 App.jsx（组合引用子组件）
- 最后 write_file 或 update_file 更新 index.html，确保包含所有组件的 script 标签
...
- [ ] **每个独立的 UI 模块/区块必须拆成单独的 .jsx 文件**
  - ❌ **错误做法**：把 UserProfile、AccountSettings、OrderHistory 全部写在 App.jsx 里（超过 100 行就必须拆分）
  - ✅ **正确做法**：每个功能模块一个文件，App.jsx 只负责组合引用

**App.jsx 的唯一职责：**
- 定义 THEME 常量
- 用 ReactDOM.render 渲染根组件
- 引用其他子组件来组装页面
- 🚫 **禁止在 App.jsx 中实现子组件的完整代码**

**正确的项目结构示例：**
\`\`\`
components/
├── App.jsx              # 根组件（只做组合，< 80 行）
├── UserProfile.jsx      # 用户信息模块
├── AccountSettings.jsx  # 账户设置模块
├── OrderHistory.jsx     # 订单历史模块
└── Header.jsx           # 头部导航
\`\`\`
...
- [ ] **组件共享**：每个组件文件末尾通过 \`Object.assign(window, { ComponentName })\` 导出
